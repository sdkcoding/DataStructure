...
void RBTree::draw_tree_hor2(Node *&ptr, int depth, char path[], int right)
{
	// stopping condition
	if (ptr == nullptr)
		return;
	...
	draw_tree_hor2(ptr->right, depth, path, 1);
	...
	// go to left node
	draw_tree_hor2(ptr->left, depth, path, 0);
}

void RBTree::draw_tree_hor()
{
	// should check if we don't exceed this somehow..
	char path[1000] = { 0 };

	//initial depth is 1 
	// --> depth를 0으로 하면 path 배열 오버플로우 발생(루트 노드 출력시 잘못된 인덱스 접근)
	draw_tree_hor2(root, 1, path, 0);
	printf("\n");
}
